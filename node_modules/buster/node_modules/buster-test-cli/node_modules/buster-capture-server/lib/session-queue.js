    ...
    addSlave: function (slave) {
        ...
        slave.prepare().then(function () {
            ...
            if (this.currentSession && this.currentSession.joinable !== false) {
                ...
                slave.loadSession(this.currentSession).then(function () {
                    this._slavesForCurrentSession.push(slave);
        ...
        slave.on("end", function () {
            ...
            if (this._slavesForCurrentSession.indexOf(slave) > -1) {
                ...
                this._slavesForCurrentSession.splice(this._slavesForCurrentSession.indexOf(slave), 1);
            }

            this.logger.info("Slave freed", slave.serialize());
            this.emit("slave:freed", slave);

            if (this._slavesForCurrentSession.length == 0 && this.currentSession && this.currentSession.joinable === false) {
